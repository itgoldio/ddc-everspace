...
def deploy_nft_root(setcodemultisig, name, symbol):
    name = ts4.str2bytes(name)
    symbol = ts4.str2bytes(symbol)
    keypair = setcodemultisig.keypair
    code_nft = ts4.load_code_cell('Nft.tvc')

    nft_root = ts4.BaseContract('Collection', ctor_params = {'name': name, 'symbol': symbol, 'codeNft': code_nft, 'ownerPubkey': keypair[1]}, keypair = keypair)
    return nft_root

#Nft root methods
def mint(setcodemultisig, nft_root, name, json):
    json = ts4.str2bytes(json)
    payload = ts4.encode_message_body('Collection', 'mintNft', {"dataName": ts4.str2bytes(name), "json": json})
    ...
    event = ts4.pop_event()
    _owners = nft_root.call_getter('_owners', {})
    _balances = nft_root.call_getter('_balances', {})
    print("MAPPING!" + str(_owners))
    print("MAPPING!" + str(_balances))
...
nft_root = deploy_nft_root(setcodemultisig, "Test", "TST")
name = "Test"
json = "{}"
